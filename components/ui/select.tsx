...
function SelectTrigger({
	...
	return (
		<SelectPrimitive.Trigger
			ref={ref}
			className={cn(
				"border-input dark:bg-input/30 dark:active:bg-input/50 bg-secondary-dark flex h-10 flex-row items-center justify-between gap-2 rounded-md border px-3 py-2 shadow-sm shadow-black/5 sm:h-9",
				Platform.select({
				...
				size === "lg" && "h-14 py-2 px-4 sm:py-1.5",
				className
			)}
			{...props}
		>
					...
					<TextClassContext.Provider value='text-foreground'>
						...
						>
							<SelectPrimitive.Content
								className={cn(
									"bg-secondary-dark border-border relative z-50 min-w-[8rem] rounded-md border shadow-md shadow-black/5",
									Platform.select({
										web: cn(
...
function NativeSelectScrollView({
	className,
	...props
}: Readonly<React.ComponentProps<typeof ScrollView>>) {
	if (Platform.OS === "web") {
